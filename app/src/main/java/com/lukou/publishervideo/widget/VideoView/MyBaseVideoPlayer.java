...
public abstract class MyBaseVideoPlayer extends MyVideoController {
    //保存系统状态ui
    ...
    protected void init(Context context) {
        super.init(context);
        mSmallClose = findViewById(com.shuyu.gsyvideoplayer.R.id.small_close);
    }
     ...
     * @param to
     */
    protected void cloneParams(MyBaseVideoPlayer from, MyBaseVideoPlayer to) {
     ...
     * 全屏的暂停返回的时候返回页面不黑色
     */
    private void pauseFullBackCoverLogic(MyBaseVideoPlayer gsyVideoPlayer) {
        //如果是暂停状态
     ...
     * 全屏
     */
    protected void resolveFullVideoShow(Context context, final MyBaseVideoPlayer gsyVideoPlayer, final FrameLayout frameLayout) {
        ...
        mOrientationUtils = new MyOrientationUtils((Activity) context, gsyVideoPlayer);
        mOrientationUtils.setEnable(isRotateViewAuto());
     ...
     * 恢复
     */
    protected void resolveNormalVideoShow(View oldF, ViewGroup vp, MyBaseVideoPlayer gsyVideoPlayer) {
    ...
    protected void clearFullscreenLayout() {
        ...
        if (oldF != null) {
            ...
            MyBaseVideoPlayer gsyVideoPlayer = (MyBaseVideoPlayer) oldF;
    ...
    protected void backToNormal() {
        ...
        final View oldF = vp.findViewById(getFullId());
        final MyBaseVideoPlayer gsyVideoPlayer;
        if (oldF != null) {
            gsyVideoPlayer = (MyBaseVideoPlayer) oldF;
            //如果暂停了
    ...
    protected Runnable mCheckoutTask = new Runnable() {
        @Override
        public void run() {
            MyBaseVideoPlayer gsyVideoPlayer = getFullWindowPlayer();
            if (gsyVideoPlayer != null
    ...
    public MyBaseVideoPlayer startWindowFullscreen(final Context context, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            MyBaseVideoPlayer.this.getClass().getConstructor(Context.class, Boolean.class);
        } catch (Exception e) {
        ...
        try {
            //通过被重载的不同构造器来选择
            Constructor<MyBaseVideoPlayer> constructor;
            final MyBaseVideoPlayer gsyVideoPlayer;
            if (!hadNewConstructor) {
                constructor = (Constructor<MyBaseVideoPlayer>) MyBaseVideoPlayer.this.getClass().getConstructor(Context.class);
                gsyVideoPlayer = constructor.newInstance(getActivityContext());
            } else {
                constructor = (Constructor<MyBaseVideoPlayer>) MyBaseVideoPlayer.this.getClass().getConstructor(Context.class, Boolean.class);
     ...
     */
    @SuppressWarnings("ResourceType, unchecked")
    public MyBaseVideoPlayer showSmallVideo(Point size, final boolean actionBar, final boolean statusBar) {
        ...
        try {
            Constructor<MyBaseVideoPlayer> constructor = (Constructor<MyBaseVideoPlayer>) MyBaseVideoPlayer.this.getClass().getConstructor(Context.class);
            MyBaseVideoPlayer gsyVideoPlayer = constructor.newInstance(getActivityContext());
    ...
    public void hideSmallVideo() {
        final ViewGroup vp = getViewGroup();
        MyBaseVideoPlayer gsyVideoPlayer = (MyBaseVideoPlayer) vp.findViewById(getSmallId());
        removeVideo(vp, getSmallId());
        mCurrentState = getGSYVideoManager().getLastState();
        if (gsyVideoPlayer != null) {
            cloneParams(gsyVideoPlayer, this);
     ...
     */
    @SuppressWarnings("ResourceType")
    public MyBaseVideoPlayer getFullWindowPlayer() {
        ...
        MyBaseVideoPlayer gsyVideoPlayer = null;
        if (full != null) {
            gsyVideoPlayer = (MyBaseVideoPlayer) full;
        }
        return gsyVideoPlayer;
    }
     ...
     */
    @SuppressWarnings("ResourceType")
    public MyBaseVideoPlayer getSmallWindowPlayer() {
        ViewGroup vp = (ViewGroup) (CommonUtil.scanForActivity(getContext())).findViewById(Window.ID_ANDROID_CONTENT);
        final View small = vp.findViewById(getSmallId());
        MyBaseVideoPlayer gsyVideoPlayer = null;
        if (small != null) {
            gsyVideoPlayer = (MyBaseVideoPlayer) small;
        }
        return gsyVideoPlayer;
    }
     ...
     * 获取当前长在播放的播放控件
     */
    public MyBaseVideoPlayer getCurrentPlayer() {
